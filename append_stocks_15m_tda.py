...
def tda_append_15m(ticker, filename):
    ...
    r = c.get_price_history_every_fifteen_minutes(ticker, start_datetime=datetime.datetime.now(),
                                                  end_datetime=datetime.datetime.now())

    try:
        x = (json.dumps(r.json(), indent=4))
        df = pd.read_json(x)
        # if getting more than one data uncomment this too
...
directory = rf'\\TOWER\financial data\15m Data Stocks'
